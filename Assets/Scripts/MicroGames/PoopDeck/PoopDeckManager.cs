...
public class PoopDeckManager : MonoBehaviour
{
    [Header("Base")]
    [SerializeField, Tooltip("The mesh renderer of the static poop deck")]
    private MeshRenderer meshRendererBase;
    [SerializeField, Tooltip("The mesh renderer of the poop deck on the moving ship")]
    private MeshRenderer meshRendererMirror;
    ...
    [SerializeField]
    private float birdPooInterval = 1f;

    private int totalMoppedScore = 0;
    ...
    public static PoopDeckManager current
    {
        get
        {
            var curr = FindObjectOfType<PoopDeckManager>();

            if (curr == null)
            {
                throw new System.Exception($"No {typeof(PoopDeckManager).Name} found in scene, please add one and set it up");
            }
    ...
    void Start()
    {
        ...
        InvokeRepeating("RandomBirdPoo", 1f, birdPooInterval);
    }
    ...
    void SetupTexture()
    {
        ...
        Texture2D original = meshRendererBase.material.mainTexture as Texture2D;
        ...
        if (debugMode)
        {
            meshRendererBase.material.mainTexture = scoreTexture;
            meshRendererMirror.material.mainTexture = scoreTexture;
        }
        else
        {
#endif
            meshRendererBase.material.mainTexture = albedoTexture;
            meshRendererMirror.material.mainTexture = albedoTexture;
#if UNITY_EDITOR
        }
#endif
    ...
    //TODO Make this more performant, it's shit rn
    Color[] SetCircle(Texture2D texture, int x, int y, float circleSize, Color color)
    {
        List<Color> coloursOverridden = new List<Color>(); //! I reckon this is what's causing the abysmal performance

        // Get the square area
        int xMin = Mathf.Clamp((int)(x - circleSize), 0, texture.width);
        int xMax = Mathf.Clamp((int)(x + circleSize), 0, texture.width);
        int yMin = Mathf.Clamp((int)(y - circleSize), 0, texture.height);
        int yMax = Mathf.Clamp((int)(y + circleSize), 0, texture.height);
        ...
        for (int i = xMin; i < xMax; i++)
        {
            for (int j = yMin; j < yMax; j++)
            {
                ...
                if (((x - i) * (x - i)) + ((y - j) * (y - j)) < circleSize * circleSize) // Not sure if it's a step up, but I reckon it'll help
                {
    ...
    void RandomBirdPoo()
    {
        ...
        SetDebris(birdPoo[Random.Range(0, birdPoo.Length)], (int)(randomPositionX * albedoTexture.width), (int)(randomPositionY * albedoTexture.height));
    }
    ...
    Color MergeColour(Color a, Color b)
    {
        ...
        newColor.a = MergeColourComponent(a.a, b.a, a.a, b.a);
    ...
    public int Mop(Ray ray)
    {
        ...
        // Convert the ray from the mirror position to the base position
        //ray.origin = meshRendererBase.transform.TransformPoint(meshRendererMirror.transform.InverseTransformPoint(ray.origin));

        // Cast that shit
        if (Physics.Raycast(ray, out RaycastHit hit))
        {
            // If we don't hit the renderer associated with this, then we don't want to do anything
            if (hit.collider.GetComponent<MeshRenderer>() != meshRendererBase) return 0;
